  ...
  while (!eof) {
    ...
    switch (header.at(0)) {
      case 0x00: {
        ...
        if (!res.has_value()) {
          spw_rmap::debug::debug("Failed to receive packet data of type 0x00");
          return std::unexpected(res.error());
        }
        total_size += *res;
        eof = true;
      ...
      case 0x02: {
        auto res = recvExact_(recv_buffer.first(*dataLength));
        if (!res.has_value()) {
          spw_rmap::debug::debug("Failed to receive packet data of type 0x02");
          return std::unexpected(res.error());
        }
        total_size += *res;
...
auto SpwRmapTCPNodeServer::poll() noexcept
  ...
  if (!res.has_value()) {
    ...
    spw_rmap::debug::debug(
        "SpwRmapTCPNodeServer::poll: Error in receiving/parsing packet: ",
        res.error().message());
    return std::unexpected{res.error()};
  }
  if (res.value() == 0) {
    ...
    case PacketType::Read: {
      ...
      if (on_read_callback_) {
        try {
          ...
          spw_rmap::debug::debug("Exception in on_read_callback_: ", e.what());
          return std::unexpected{
    ...
    case PacketType::Write: {
      if (on_write_callback_) {
        try {
          ...
          spw_rmap::debug::debug("Exception in on_write_callback_: ", e.what());
          return std::unexpected{
              std::make_error_code(std::errc::operation_canceled)};
        }
      }
...
auto SpwRmapTCPNodeServer::runLoop() noexcept
...
auto SpwRmapTCPNodeServer::send_(size_t total_size)
  ...
  return tcp_server_->sendAll(std::span(send_buf_).first(total_size + 12));
}
...
auto SpwRmapTCPNodeServer::sendWritePacket_(
  ...
  if (!tcp_server_) {
    spw_rmap::debug::debug(
        "SpwRmapTCPNodeServer::sendWritePacket_: Not connected");
    return std::unexpected{std::make_error_code(std::errc::not_connected)};
  }
...
auto SpwRmapTCPNodeServer::sendReadPacket_(
  ...
  if (!tcp_server_) {
    spw_rmap::debug::debug(
        "SpwRmapTCPNodeServer::sendReadPacket_: Not connected");
    return std::unexpected{std::make_error_code(std::errc::not_connected)};
  }
  auto expected_length = target_node->getTargetSpaceWireAddress().size() +
                         (target_node->getReplyAddress().size() + 3) / 4 * 4 +
...
auto SpwRmapTCPNodeServer::emitTimeCode(uint8_t timecode) noexcept
    -> std::expected<std::monostate, std::error_code> {
  if (!tcp_server_) {
    spw_rmap::debug::debug("SpwRmapTCPNodeServer::emitTimeCode: Not connected");
    return std::unexpected{std::make_error_code(std::errc::not_connected)};
  }
